package com.tripod.durust.presentation.login.emailverification
...
import com.tripod.durust.domain.AccountServiceImpl
...
class EmailVerificationViewModel(val auth: FirebaseAuth): ViewModel() {
    ...
    fun sendVerificationEmail(snackbarHostState: SnackbarHostState){
        ...
        viewModelScope.launch {
            AccountServiceImpl().sendEmailVerificationLink(auth, email){
                task ->
    ...
    private fun reAuthenticateUser(email: String, password: String,snackbarHostState: SnackbarHostState,onSuccessful: () -> Unit){
        ...
        viewModelScope.launch {
            AccountServiceImpl().reauthenticateAfterEmailVerification(auth, email = email, password = password){
                task ->
                if(task.isSuccessful){
